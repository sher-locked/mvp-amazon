...
- `Tag` — atomic claim on two axes: `type` (fact|functional|sensory|emotional|occasion|audience) × `scope` (brand|category|family|sku), stored once at its highest true scope (see `docs/reference/sku-tags.md`); target vocabulary renames these to **Claims** with optional buckets
